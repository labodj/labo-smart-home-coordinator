...
describe("LshCodec", () => {
  ...
  describe("decode", () => {
    ...
    it("preserves already parsed JSON objects from auto-detecting adapters", () => {
      const payload = { p: 1, n: "device1" };

      expect(codec.decode(payload, "json")).toBe(payload);
      expect(codec.decode(payload, "text")).toBe(payload);
    });
