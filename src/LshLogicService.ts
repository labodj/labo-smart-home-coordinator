/**
 * @file Core business logic for the standalone LSH coordinator.
 * The LshLogicService class orchestrates the various managers (`DeviceRegistryManager`, etc.)
 * and is completely decoupled from transport/runtime adapters. It returns descriptive
 ...
 * @description The main service class for LSH coordination. It contains all business logic
 * and state, but does not interact directly with MQTT clients or host runtimes.
...
export class LshLogicService {
  ...
  private _handleRetainedHomieState(
    ...
    // runtime transitions `lost -> init -> ready` can also reach this process with
    // `msg.retain === true`.
    //
  ...
  private _handleLiveHomieState(
    ...
    // `lost` means "device went offline", even if this coordinator session never observed
